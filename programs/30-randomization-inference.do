...
*   - Headline: matched_per_100k, full sample (Figure 5)
...
log using "${topdir}/output/30-randomization-inference.log", replace
...
* Merge the Medicaid GME formula classification (2015 payment rules) for the mech specs
preserve
...
program define _avgatt, rclass
    args cohortvar
    capture noisily did_imputation matched_per_100k program_numeric_id year `cohortvar' ///
...
program define _rispec
    ...
    * state_id values present in the (possibly restricted) sample, not to
    * state_id = 1..Ns (which is only correct for the full sample; restricted
    * samples have gaps in state_id). We pair the cohort list with the sample's
    * own state_ids in random order (a uniform permutation).
    local ge = 0
    local valid = 0
    forval r = 1/`reps' {
        preserve
...
use "`ri_file'", clear
list, clean noobs
export delimited using "${tabdir}/randomization-inference-summary.csv", replace

di ""
di "=================================================================="
di "Randomization inference complete (REPS = `REPS')."
di "Summary: ${tabdir}/randomization-inference-summary.csv"
di "=================================================================="
