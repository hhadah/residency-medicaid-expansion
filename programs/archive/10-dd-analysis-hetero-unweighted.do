/*
Heterogeneity Analysis by Urban/Rural Classification (UNWEIGHTED)
...
NOTE: This version does NOT use population weights
*/
...
* Open log file
log using "${topdir}/output/10-dd-analysis-hetero-unweighted.log", replace
...
*===============================================================================
* Heterogeneity Analysis - Urban vs Rural (UNWEIGHTED)
*===============================================================================

di ""
di "========================================================================="
di "HETEROGENEITY ANALYSIS (UNWEIGHTED): By Urban/Rural Classification"
di "Urban vs Rural"
di "========================================================================="
di ""
...
foreach outcome of global outcomes {
    ...
    di ""
    di "========================================================================="
    di "HETEROGENEITY ANALYSIS (UNWEIGHTED): ${label_`outcome'}"
    di "Outcome variable: `outcome'"
    di "========================================================================="
    di ""
    
    * Run did_imputation with heterogeneous effects by urban/rural (0 = urban, 1 = rural), UNWEIGHTED
    ...
    * Calculate baseline means (pre-treatment observations in treated states, UNWEIGHTED)
    quietly summarize `outcome' if urban_rural == 0 & treated_state == 1 & year < year_expanded
    local baseline_urban = r(mean)
    if missing(`baseline_urban') | `baseline_urban' == 0 {
        local baseline_urban = 1
    }
    
    quietly summarize `outcome' if urban_rural == 1 & treated_state == 1 & year < year_expanded
    ...
    * Calculate dynamic annotation coordinates using maximum upper confidence intervals from both groups
    ...
    local plot_title "Heterogeneity (Unweighted): Urban vs Rural - `label'"
    ...
    * Create combined plot using twoway, with annotation
    twoway (rarea urb_ci_upper urb_ci_lower urb_period if urb_pre, fcolor("31 119 180%20") lcolor("31 119 180%20") lwidth(none)) ///
           ...
           text(`y_annot' `x_annot' `"Urban Baseline: `text_baseline_urban' | Effect: `text_urban' (`text_pct_urban'%, p=`text_urban_p')"' `"Rural Baseline: `text_baseline_rural' | Effect: `text_rural' (`text_pct_rural'%, p=`text_rural_p')"' `"Difference: `text_avg_diff' (p=`text_avg_diff_p')"', size(small)) ///
    ...
    graph export "${figdir}/`prefix'-hetero_urbanrural_`short'_event_unweighted.png", as(png) replace width(1200) height(800)
    graph export "${latex_figdir}/`prefix'-hetero_urbanrural_`short'_event_unweighted.png", as(png) replace width(1200) height(800)
...
di ""
di "========================================================================="
di "HETEROGENEITY ANALYSIS BY URBAN/RURAL CLASSIFICATION (UNWEIGHTED) COMPLETED"
di "Event study plots saved to:"
di "  - ${figdir}/221-hetero_urbanrural_*_event_unweighted.png"
di "  - ${latex_figdir}/221-hetero_urbanrural_*_event_unweighted.png"
di "========================================================================="
