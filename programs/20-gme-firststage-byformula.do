...
* ---------------------------------------------------------------------------
* Closes the mechanism chain. The pooled first stage (script 16) shows expansion
...
*
* This re-runs the DGME and IME event studies from script 16 separately for:
...
* merged from data/raw/gme_formula_classification.csv (as in script 13).
*
...
log using "${topdir}/output/20-gme-firststage-byformula.log", replace

* -------------------------------------------------------------------------
* Load and collapse the GME funding panel (identical to script 16)
* -------------------------------------------------------------------------
...
tempname fs
tempfile fs_file
postfile `fs' str20 outcome str12 group double avg_treat treat_p pretrend_p ///
...
foreach outcome in asinh_dgme asinh_ime asinh_dgme_ftes asinh_dgme_perfte {
    foreach grp in volume notvolume {
        ...
        * Average post ATT (tau0..5) and joint tests
        local tau_sum = 0
        local tau_n = 0
        forval h = 0/5 {
        ...
        post `fs' ("`outcome'") ("`grp'") (`avg_treat') (`treat_p') (`pretrend_p') ///
            (`baseline') (`pct') (`nh') (`ns')
